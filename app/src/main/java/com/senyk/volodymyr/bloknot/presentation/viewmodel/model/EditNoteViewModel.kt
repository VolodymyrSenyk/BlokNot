...
import com.senyk.volodymyr.bloknot.presentation.view.fragment.EditNoteFragmentDirections
...
class EditNoteViewModel @Inject constructor(
    ...
    fun onStart(noteId: String?) {
        if (noteId != null) {
            subscribe(
                ...
                onSuccess = { note ->
                    _note.value = noteUiDtoMapper(note)
                }
            )
        }
    }
    ...
    fun onConfirmNoteDeleteClick(noteId: String) {
        if (noteId.isEmpty()) {
            _navigationEvent.setHandledValue(NavigateBackToFragmentEvent(R.id.notesListFragment))
        } else {
            subscribe(
    ...
    private fun navigateToNoteDetailsFragment(noteId: String) {
        val action =
            EditNoteFragmentDirections.actionEditNoteFragmentToNoteDetailsFragment(
                noteId = noteId
            )
